...
import React, { useState, useEffect } from 'react';
import { motion, AnimatePresence, useScroll, useMotionValueEvent } from 'framer-motion';
import { Menu, X, User, LogOut, Phone, Mail, UserCircle } from 'lucide-react';
      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-4">
            <span className="text-yellow-400">Nubra Valley, Leh Ladakh</span>
            {user && (
              <motion.div
              ...
              className={`hidden lg:flex items-center space-x-2 px-6 py-2.5 rounded-full font-medium transition-all duration-300 ${
                isScrolled 
                  ? user
                    ...
                    : 'bg-yellow-500 text-white hover:bg-yellow-600 shadow-lg hover:shadow-xl'
                  : user
                    ? 'bg-red-500/90 text-white border border-red-400/30 hover:bg-red-600 backdrop-blur-sm'
                    : 'bg-white/10 text-white border border-white/20 hover:bg-white hover:text-gray-900 backdrop-blur-sm'
              }`}
            >
              {user ? <LogOut size={16} /> : <User size={16} />}
              <span className="text-sm">
                {user ? `Sign Out (${getUserDisplayName()})` : 'Book Now'}
              </span>
            </motion.button>
            ...
            >
              <div className="p-8">
                ...
                <motion.button
                  ...
                  className={`w-full flex items-center justify-center space-x-3 py-4 px-6 rounded-xl font-semibold transition-colors duration-300 ${
                    user
                ...
                >
                  {user ? <LogOut size={18} /> : <User size={18} />}
                  <span>{user ? `Sign Out (${getUserDisplayName()})` : 'Book Now'}</span>
                </motion.button>
